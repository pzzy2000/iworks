...
public class GenericConverterFactory {
       ...
       * sourceType to the given targetType. This method first searches to see
       * if any of the registered converters are capable of making this
       * conversion alone. If one is found, it is returned. Otherwise, this
       * method will search to see if some combination of registered converters
       * can be used to perform this conversion. If so, a composite converter
       * will be returned.
       * <P>
      ...
      public ITypeConverter getConverter(Class<?> sourceType, Class<?> targetType) {
            ...
            // we are going to start searching to see if some chain of
            // converters
            ...
            if (converter == null && _reverseDiscoveryConverters.size() > 0) {
                  ...
                  // we do not find a converter dynamically for the target type
                  // when
                  // the source type is
                  // not convertable from the target type (converter's
                  // availability
                  // has to be 'reflective').
                  // Regular generic converters are pre-defined, so they
                  // normally do
       ...
       * 
       * @param sourcesToBeSearched
       *              each element is a Node. Each Node is a pairing of
       *              sourceType and the chain of converters needed to produce
       *              this sourceType.
       * @param targetType
       *              the type that is needed
       * @param cache
       *              used to record which classes we've searched already.
       * @param useRevserseDiscovery
       *              true if "reverse discovery" converters should be used,
       *              false otherwise
       * @return null if no converter was found.
       */
      private ITypeConverter _findConverter(LinkedList<Node> sourcesToBeSearched, Class<?> targetType, Set<Class<?>> cache, boolean useRevserseDiscovery) {
            while (!sourcesToBeSearched.isEmpty()) {
                  Node source = sourcesToBeSearched.removeFirst();
                  ITypeConverter match = null;

                  // loop through all the converters and see what types they can
                  // turn
                  // the current sourceType into
                  // (the current sourceType is source.targetType):
                  for (IGenericConverter conv : _converters) {
                        // loop though all the targetTypes on this converter to
                        // see
      ...
      /**
       * Searches the targetTypes of the given converter to see if we can find
       ...
       *              this sourceType.
       * @param currentSource
       *              a chain of converters has been used to produce the type
       *              identified by this Node. The targetType of this Node will
       *              be used to search the currentConverter.
       * @param searchType
       *              the targetType we are searching for.
       * @param cache
       *              used to record which classes we've searched already.
       * @return true if the currentConverter can convert from
       *         currentSource.targetType into searchType.
       */
      private boolean _searchTargetTypes(List<Node> sourcesToBeSearched, Node currentSource, IGenericConverter currentConverter, Class<?> searchType, Set<Class<?>> cache) {
            ...
            for (int i = 0, sz = targetTypes.size(); i < sz; i++) {
                  ...
                  if (cache.add(targetType)) {
                        ...
                        // create a new node in the chain by adding this
                        // targetType and
                        // converter
                        Node newSource = new Node(currentSource, currentConverter, targetType);

                        // add the new node so that we can continue searching by
                        // seeing
                        // if
                        // we can convert the targetType into the searchType
                        // using some
                        // other
                        // converter:
