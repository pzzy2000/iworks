...
public class ObjectMappingCache {
      ...
      private void validate(Class<?> cls, Field field) {
            ...
            if (column.columnType().getJdbcType().equals(JDBCType.BINARY) || column.columnType().getJdbcType().equals(JDBCType.VARBINARY)
                                                || column.columnType().getJdbcType().equals(JDBCType.LONGVARBINARY)) {
                  // throw new RuntimeException(
                  // "this JDBCType : JDBCType.BINARY JDBCType.VARBINARY
                  // JDBCType.LONGVARBINARY no
                  // support ! ");

                  throw new RuntimeException("class " + cls.getName() + "  field : " + field.getName() + " type : " + field.getType() + "  no equals  jdbc type " + column.columnType().getJavaType()
                  ...
                  } else if (field.getType().equals(Date.class) && column.columnType().getJavaType().equals(Timestamp.class)) {

                  } else {
                        if (!column.columnType().getJavaType().equals(field.getType())) {
                              throw new RuntimeException("class " + cls.getName() + "  field : " + field.getName() + " type : " + field.getType() + "  no equals  jdbc type "
                                                                  + column.columnType().getJavaType() + " this field type : " + column.columnType().getJavaType());
                        }
                  }
            ...
            if (iEntityMapCache == null) {
                  synchronized (cls) {
                        ...
                        if (iEntityMapCache == null) {
                              ...
                              // logger.info("entity cache sacn class : " +
                              // cls.getName() + " table " + table);
