...
/**
 * <p>
 * A {@link org.apache.shiro.cache.Cache} implementation backed by Redisson
 * instance.
 * </p>
 *
 * @author streamone
...
public class RedissonShiroCache<K, V> implements Cache<K, V> {
      ...
      public V put(K key, V value) throws CacheException {
            ...
            if (mapCache != null) {
                  previous = mapCache.put(key, val, config.getTTL(), TimeUnit.MILLISECONDS, config.getMaxIdleTime(), TimeUnit.MILLISECONDS);
            } else {
      ...
      public void fastPut(K key, V value) throws CacheException {
            ...
            Object val = toStoreValue(value);
            if (mapCache != null) {
                  mapCache.fastPut(key, val, config.getTTL(), TimeUnit.MILLISECONDS, config.getMaxIdleTime(), TimeUnit.MILLISECONDS);
            } else {
                  map.fastPut(key, val);
            }
      }
      ...
      public V putIfAbsent(K key, V value) throws CacheException {
            ...
            Object val = toStoreValue(value);
            if (mapCache != null) {
                  previous = mapCache.putIfAbsent(key, val, config.getTTL(), TimeUnit.MILLISECONDS, config.getMaxIdleTime(), TimeUnit.MILLISECONDS);
            } else {
      ...
      public boolean fastPutIfAbsent(K key, V value) throws CacheException {
            ...
            Object val = toStoreValue(value);
            if (mapCache != null) {
                  return mapCache.fastPutIfAbsent(key, val, config.getTTL(), TimeUnit.MILLISECONDS, config.getMaxIdleTime(), TimeUnit.MILLISECONDS);
            } else {
