...
import static com.github.streamone.shiro.session.RedissonSession.INFO_HOST_KEY;
import static com.github.streamone.shiro.session.RedissonSession.INFO_ID_KEY;
import static com.github.streamone.shiro.session.RedissonSession.INFO_LAST_KEY;
import static com.github.streamone.shiro.session.RedissonSession.INFO_START_KEY;
import static com.github.streamone.shiro.session.RedissonSession.INFO_STOP_KEY;
import static com.github.streamone.shiro.session.RedissonSession.INFO_TIMEOUT_KEY;

/**
 * <p>
 * Redis lua scripts for session operations.
 * </p>
 *
 * @author streamone
 */
public abstract class RedissonSessionScript {
	...
	public static final String TOUCH_SCRIPT = "if redis.call('PTTL', KEYS[1]) <= 0 then\n" + "  return " + makeError(RETURN_CODE_EXPIRED)
			...
			+ makeError(RETURN_CODE_STOPPED) + "\n" + "end\n" + "\n" + "local timeoutEncoded = redis.call('HGET', KEYS[1], '\""
			+ INFO_TIMEOUT_KEY + "\"')\n" + "if timeoutEncoded == nil then\n" + "  return " + makeError(RETURN_CODE_INVALID) + "\n"
			+ "end\n" + "\n" + "local timeout = cjson.decode(timeoutEncoded)[2]\n" + "\n" + "redis.call('HSET', KEYS[1], '\""
			+ INFO_LAST_KEY + "\"', ARGV[1])\n" + "redis.call('PEXPIRE', KEYS[1], timeout)\n" + "redis.call('PEXPIRE', KEYS[2], timeout)";

	public static final String INIT_SCRIPT = "redis.call('HMSET', KEYS[1], '\"" + INFO_ID_KEY + "\"', ARGV[1], '\"" + INFO_TIMEOUT_KEY
	...
	public static final String GET_START_SCRIPT = "if redis.call('PTTL', KEYS[1]) <= 0 then\n" + "  return "
			+ makeError(RETURN_CODE_EXPIRED) + "\n" + "end\n" + "\n" + "if redis.call('HEXISTS', KEYS[1], '\"" + INFO_STOP_KEY
			+ "\"') == 1 then\n" + "  return " + makeError(RETURN_CODE_STOPPED) + "\n" + "end\n" + "\n"
			+ "local startTime = redis.call('HGET', KEYS[1], '\"" + INFO_START_KEY + "\"')\n" + "if startTime == nil then\n" + "  return "
			+ makeError(RETURN_CODE_INVALID) + "\n" + "end\n" + "\n" + "return startTime";
	...
	public static final String SET_TIMEOUT_SCRIPT = "if redis.call('PTTL', KEYS[1]) <= 0 then\n" + "  return "
			...
			+ "\"') == 1 then\n" + "  return " + makeError(RETURN_CODE_STOPPED) + "\n" + "end\n" + "\n"
			+ "local timeout = redis.call('HGET', KEYS[1], '\"" + INFO_TIMEOUT_KEY + "\"')\n" + "if timeout == nil then\n" + "  return "
			+ makeError(RETURN_CODE_INVALID) + "\n" + "end\n" + "\n" + "redis.call('HSET', KEYS[1], '\"" + INFO_TIMEOUT_KEY
