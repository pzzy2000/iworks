...
/**
 * <p>
 * A {@link org.apache.shiro.session.mgt.SessionManager} implementation backed
 * by Redisson Objects.
 * </p>
 *
 * @author streamone
...
public class RedissonSessionManager extends AbstractNativeSessionManager {
      ...
      protected Session doGetSession(SessionKey key) throws InvalidSessionException {
            ...
            if (sessionId == null) {
                  if (logger.isDebugEnabled()) {
                        logger.debug("Unable to resolve session ID from SessionKey [{}].  Returning null to indicate a " + "session could not be found.", key);
                  }
                  return null;
            }
            Session s = sessionDAO.readSession(sessionId);
            if (s == null) {
                  // session ID was provided, meaning one is expected to be
                  // found, but we couldn't
                  // find one:
