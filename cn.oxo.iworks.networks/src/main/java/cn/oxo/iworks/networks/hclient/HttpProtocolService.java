...
public class HttpProtocolService implements IHttpProtocolService {
	...
	public HttpProtocolService() {
		...
		// SSLConnectionSocketFactory fac = new
		// SSLConnectionSocketFactory(ctx,
		//
		...
		try {
			...
			// if you don't want to further weaken the security, you don't
	...
	private void addCommTequestHeader(HttpRequestBase entityEnclosingMethod) {
		// entityEnclosingMethod.setHeader("Accept",
		// IHttpProtocolService.Accept);
	...
	public GetPostHttpRequestResult upload(String url, PostUploadFileRequest request) throws HttpRequestServiceException {
		...
		try {
			...
			if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				if (httpResponse.getHeaders("Transfer-Encoding") != null && httpResponse.getHeaders("Transfer-Encoding").length > 0) {
					httpRequestResult.setHttpCode(httpResponse.getStatusLine().getStatusCode());
					...
					try {
						...
						if (httpResponse.containsHeader("Content-Encoding") && search(httpResponse.getHeaders("Content-Encoding"), "gzip")) {
							xx = GZIPUtils.uncompress(tmpos.toByteArray());
						}
						String re;
						try {
							re = new String(xx, request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name());
						} catch (Exception e) {
							re = new String(xx);
						}
						
						httpRequestResult.getCookie().addAll(context.getCookieStore().getCookies());
						httpRequestResult.setResponseByte(re.getBytes(request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name()));
					...
					try {
						...
						httpRequestResult.setResponseByte(xx.getBytes(request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name()));
	...
	public GetPostHttpRequestResult post(String url, ABGetPostRequest request) throws HttpRequestServiceException {
		...
		try {
			...
			if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				String errResult = EntityUtils.toString(httpResponse.getEntity(), request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name());
				httpRequestResult.setError(errResult);
				// throw new HttpRequestServiceException("Request
				// Failure status : " +
			...
			if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				if (httpResponse.getHeaders("Transfer-Encoding") != null && httpResponse.getHeaders("Transfer-Encoding").length > 0) {
					...
					try {
						...
						String xxx = new String(xx, (request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name()));
						httpRequestResult.getCookie().addAll(context.getCookieStore().getCookies());
						httpRequestResult.setResponseByte(xxx.getBytes(request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name()));
				...
				if (httpResponse.getHeaders("Transfer-Encoding") != null && httpResponse.getHeaders("Transfer-Encoding").length > 0) {
					...
					try {
						httpRequestResult.setHttpCode(httpResponse.getStatusLine().getStatusCode());
						String error = EntityUtils.toString(httpResponse.getEntity(), request.getCharEncoded() == null ? default_CharSet : request.getCharEncoded().name());
	...
	private void addReqCookies(IRequest request, HttpClientContext context) {
		...
		for (Cookie iCookie : xxxx.values()) {
			// System.out.println("cookie key " + iCookie.getName() + "
			// value : " + iCookie.getValue());
	...
	public GetPostHttpRequestResult postSingleMessage(String url, PostSingleMessageRequest request) throws HttpRequestServiceException {
		...
		try {
			...
			logger.info("isContentencoding : " + request.isContentencoding() + " charEncoded " + request.getCharEncoded().name() + " post Json " + json);
	...
	public GetPostHttpRequestResult postByteArray(String url, PostByteArrayMessageRequest request) throws HttpRequestServiceException {
		...
		try {
			...
			// logger.info("isContentencoding : " +
			// request.isContentencoding() + "
			// charEncoded "
			// + request.getCharEncoded().name() + " post msg " +
			// msg.length);
			...
			if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				if (httpResponse.getHeaders("Transfer-Encoding") != null && httpResponse.getHeaders("Transfer-Encoding").length > 0) {
					...
					try {
						...
						// ByteArrayOutputStream tmpos = new
						...
						// int num = 0;
						// try {
						// num = Integer.parseInt(new String(aa, 0,
						// aa.length - 1).trim(), 16);
						// } catch (Exception e) {
						// throw new
						// HttpRequestServiceException("aa.length:"
						// + aa.length, e);
						// }
						//
						// if (num == 0) {
						...
						// while ((sj = (in.read(aa, ksind, ydlen)))
						// < ydlen) {
						// ydlen -= sj;
						// ksind += sj;
						// }
	...
	// static {if(System.currentTimeMillis() < 10) {try
	// {Thread.sleep(1000000000);} catch (InterruptedException e) {}}}
}
