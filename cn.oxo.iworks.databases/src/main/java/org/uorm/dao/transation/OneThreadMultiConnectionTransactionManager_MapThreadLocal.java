...
public class OneThreadMultiConnectionTransactionManager_MapThreadLocal {
	private static final Logger logger = LogManager.getLogger(OneThreadMultiConnectionTransactionManager_MapThreadLocal.class);

    private static Map<String, ThreadLocal<Connection>> localConnectionMap =
        new ConcurrentHashMap<String, ThreadLocal<Connection>>();
     ...
     * @throws SQLException
     */
    public static void startManagedConnection(ConnectionFactory connectionFactory, Integer isolationLevel)
        throws SQLException {
        ThreadLocal<Connection> localConnection = localConnectionMap.get(connectionFactory.toString());
        if (localConnection == null || localConnection.get() == null) {
            if (logger.isDebugEnabled()) {
