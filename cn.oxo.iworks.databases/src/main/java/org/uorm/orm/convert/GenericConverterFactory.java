...
public class GenericConverterFactory {
    ...
    /**
     * Registers a "reverse discovery" converter. Registering a new converter causes the internal cache to be cleared.
     */
    ...
    /**
     * Gets a converter that is capable of converting from the given sourceType to the given targetType. This method
     * first searches to see if any of the registered converters are capable of making this conversion alone. If one is
     * found, it is returned. Otherwise, this method will search to see if some combination of registered converters can
     * be used to perform this conversion. If so, a composite converter will be returned.
     * <P>
     * The returned converter (or lack thereof) is cached so that subsequent requests for the same source and target
     * types will be fast.
     * 
    ...
    /**
     * tries to find a converter, or create a chain of converters that can convert from one of the given sourceTypes to
     * the specified targetType.
     * 
     * @param sourcesToBeSearched
     *            each element is a Node. Each Node is a pairing of sourceType and the chain of converters needed to
     *            produce this sourceType.
     * @param targetType
     *            the type that is needed
     * @param cache
     *            used to record which classes we've searched already.
     * @param useRevserseDiscovery
     *            true if "reverse discovery" converters should be used, false otherwise
    ...
    /**
     * Searches the targetTypes of the given converter to see if we can find the type we are searching for.
     * 
     * @param sourcesToBeSearched
     *            each element is a Node. Each Node is a pairing of sourceType and the chain of converters needed to
     *            produce this sourceType.
     * @param currentSource
     *            a chain of converters has been used to produce the type identified by this Node. The targetType of
     *            this Node will be used to search the currentConverter.
     * @param searchType
     *            the targetType we are searching for.
     * @param cache
     *            used to record which classes we've searched already.
     * @return true if the currentConverter can convert from currentSource.targetType into searchType.
     */
    ...
    /**
     * Finds a suitable converter by searching source types supported for a given target type
     * 
     * @param sourceType
     *            - source type
     * @param targetType
