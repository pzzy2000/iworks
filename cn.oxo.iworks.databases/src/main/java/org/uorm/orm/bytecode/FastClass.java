...
public class FastClass implements Serializable {
     ...
     *             Indicates a problem performing invocation
    ...
    public Object invoke(Object obj, Field field, Object args) throws IllegalAccessException {
        ...
        if (field.getType().isEnum()) {
            if (args != null) {
                ...
                FieldUtils.writeField(field, obj, i, true);
            } else {

            }

        } else {
            FieldUtils.writeField(field, obj, args, true);
        }
     ...
     */
    @SuppressWarnings("rawtypes")
    private int getIndex(Class<?>[] parameterTypes) {
        final Constructor[] constructors = this.type.getConstructors();
        boolean eq;
