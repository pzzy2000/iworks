...
public class FailoverHotSwapHAConnectionFactory implements ConnectionFactory {
	private static final Logger logger = LogManager.getLogger(FailoverHotSwapHAConnectionFactory.class);
    ...
    private Map<ScheduledFuture<?>, ScheduledExecutorService> schedulerFutures =
        new ConcurrentHashMap<ScheduledFuture<?>, ScheduledExecutorService>();
    ...
    public void init() {
        ...
        ScheduledFuture<?> future = scheduler.scheduleWithFixedDelay(job, 0, monitorPeriod, TimeUnit.MILLISECONDS);
    ...
    public DatasourceConfig getConfiguration() {
        return currentDataSource == null ? descriptor.getMainDataSource().getConfiguration()
            : currentDataSource.getConfiguration();
    }
    ...
    /**
     * @param detectingRequestTimeout
     *            the detectingRequestTimeout to set
     */
